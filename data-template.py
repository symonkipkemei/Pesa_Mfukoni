...
def purpose_table():
    ...
    while try_again:
        # create a table object for status
        status_table = s.Table('purpose', metadata, autoload=True, autoload_with=engine)
    
        # select entries in the database
        status_query = s.select([status_table.columns.purppose_id, status_table.columns.purpose_type])
        status_proxy = connection.execute(status_query)
